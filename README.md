...
- `sheet.cell(address)` 返回可复用的 `Cell` 句柄，值/公式/样式索引会按工作表 revision 缓存；现在也可以通过 `cell.style` / `cell.font` 读取当前样式与字体定义，并用 `cell.setStyle(patch)` / `cell.setFont(patch)` / `cell.cloneStyle(patch?)` 直接派生并应用新样式
